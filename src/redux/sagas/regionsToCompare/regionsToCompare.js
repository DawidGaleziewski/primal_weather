...
// Redux
import {fetchData, setData, fetchError} from '@Redux/features/regionsToCompare/regionsToCompare';

// # 1 Workers
function* regionsToCompareWorker(action){
    try {
        const {payload: {query, days}} = action
        const {data: {forecast: {forecastday}}} = yield call(api.getWeather, query, days);
